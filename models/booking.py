...
class BookingRoomLine(models.Model):
    _name = "hotel1.room.line"
    _description = "Hotel Room Reservation"
    _rec_name = "room_id"

    room_id = fields.Many2one("hotel1.room", ondelete="restrict", index=True)

    book_id = fields.Many2one("hotel1.booking", "Booking Number", ondelete="cascade")
...
class Booking(models.Model):
    _name = "hotel1.booking"
    name = fields.Char("Name")
    room_id = fields.One2many(
        'hotel1.room.line',
        'book_id',
        'Room',
    ...
    def create(self, vals):
        ...
        """
        if not "service_line_ids" and "book_id" in vals:
            tmp_room_lines = vals.get("room_line_ids", [])
            vals["order_policy"] = vals.get("hotel_policy", "manual")
            vals.update({"room_line_ids": []})
            folio_id = super(Booking, self).create(vals)
            for line in tmp_room_lines:
                line[2].update({"book_id": folio_id.id})
            vals.update({"room_line_ids": tmp_room_lines})
            folio_id.write(vals)
        else:
            if not vals:
                vals = {}
            vals["name"] = self.env["ir.sequence"].next_by_code("hotel1.booking.seq")
    ...
    #     hotel_room_obj = self.env["hotel1.room"]
    #     folio_room_line_obj = self.env["hotel1.room.line"]
    #     for rec in self:
    ...
    #             for rm in room_list:
    #                 room_obj = hotel_room_obj.search([("name", "=", rm.id)])
    ...
    #                 room_obj = hotel_room_obj.search([("name", "=", room.id)])
    #                 # room_obj.write({"isroom": False})
    #                 room_vals = {
    #                     "room_id": room_obj.id,
    #                     "check_in": rec.checkin_date,
    ...
    def action_cancel(self):
        ...
        #     ):
        #         rooms = self.env["hotel1.room"].search([("name", "=", product.id)])
...
class HotelBookingLine(models.Model):
    ...
    _description = "Hotel Booking Line"
    ...
    booking_id = fields.Many2one("hotel1.booking", "Booking", ondelete="cascade")
    ...
    def create(self, vals):
        ...
        """
        if "folio_id" in vals:
            folio = self.env["hotel1.booking"].browse(vals["booking_id"])
            vals.update({"order_id": folio.order_id.id})
        return super(HotelBookingLine, self).create(vals)
    ...
    def unlink(self):
        ...
        #         rooms = self.env["hotel1.room"].search(
...
class HotelServiceLine(models.Model):
    ...
    booking_id = fields.Many2one("hotel1.booking", "Folio", ondelete="cascade")
